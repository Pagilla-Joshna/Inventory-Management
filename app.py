...
def delete_user(username):
    ...
    success = delete_user_by_username(username) # type: ignore
    if success:
...
def reset_password(username):
    ...
    success = reset_user_password(username, new_password) # type: ignore
    if success:
...
def update_quantity():
    ...
    product = request.form['product']
    category = request.form['category']
    quantity_input = request.form['new_quantity'].strip()

    if not quantity_input.isdigit():
        return "Invalid quantity", 400

    qty_delta = int(quantity_input)
    ...
    # Admin adds quantity, user subtracts quantity
    if user_role == 'admin':
        df.loc[mask, 'Quantity'] = current_qty + qty_delta
    else:
